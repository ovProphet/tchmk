...
extern "C" {
	 BigInt* Big_new() {return new BigInt();}
	 void Big_read(BigInt* a, char* fname, char* bin) {\
		bool b = binary(bin);
		FILE* f = ropen(fname,b);
		a->input(f,b);
	}
	void Big_write(BigInt* a, char* fname, char* bin) {\
		bool b = binary(bin);
		FILE* f = wopen(fname,b);
		a->output(f,b);
	}
	 BigInt* Big_pow(BigInt* a,BigInt* b,BigInt*  mod)
	{
	...
	BigInt* Big_sub(BigInt* a,BigInt* b)
	{
		BigInt* res=new BigInt();
		res[0]=a[0] - b[0];
		return res;
	}
	BigInt* Big_mul(BigInt* a,BigInt* b){
		BigInt* res=new BigInt();
		res[0]=a[0] * b[0];
		return res;
	}
